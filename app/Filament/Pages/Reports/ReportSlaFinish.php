...
class ReportSlaFinish extends Page implements HasTable
{
    ...
    protected static ?string $title = 'Report SLA Finish';
            ...
            ->modifyQueryUsing(function (\Illuminate\Database\Eloquent\Builder $query, \Filament\Tables\Contracts\HasTable $livewire) {
                ...
                $query->withCount([
                    'outstandings as sla1_count' => function ($q) use ($baseFilter) {
                        $baseFilter($q);
                        $q->whereRaw('DATEDIFF(date_finish, date_visit) BETWEEN 0 AND 3');
                    },
                    'outstandings as sla2_count' => function ($q) use ($baseFilter) {
                        $baseFilter($q);
                        $q->whereRaw('DATEDIFF(date_finish, date_visit) BETWEEN 4 AND 7');
                    },
                    'outstandings as sla3_count' => function ($q) use ($baseFilter) {
                        $baseFilter($q);
                        $q->whereRaw('DATEDIFF(date_finish, date_visit) > 7');
                    },
                    'outstandings as not_finish_count' => function ($q) use ($baseFilter) {
                        $baseFilter($q);
                        $q->whereNull('date_finish');
                    },
                    'outstandings as temporary_count' => function ($q) use ($baseFilter) {
                        $baseFilter($q);
                        $q->whereNotNull('date_finish')->where('is_temporary', 1);
                    },
                ]);
            })
            ->columns([
                ...
                TextColumn::make('sla3_count')
                    ...
                    ->sortable()
                    ->summarize(Sum::make()->label('Total')),
                TextColumn::make('not_finish_count')
                    ->label('Not Finish')
                    ->sortable()
                    ->color('danger')
                    ->summarize(Sum::make()->label('Total')),
                TextColumn::make('temporary_count')
                    ->label('Temporary')
                    ->sortable()
                    ->color('warning')
                    ->summarize(Sum::make()->label('Total')),
